function trainAndTest_normalizedL2_FV(featDir_FV,featDir_LLC)
    ...
    for j = 1:25
    	classAndwordTerm = [];
    	featFile{j} = [fullfile(featDir_FV,sprintf('/cw/%d.mat',j))];
    	SingleclassAndwordTerm = dlmread(featFile{j});
	classAndwordTerm = [classAndwordTerm; SingleclassAndwordTerm];
    end
    for j = 1:25
	
	testclassAndwordTerm = classAndwordTerm[j];
	classAndwordTerm[j] = [];
	trainclassAndwordTerm = classAndwordTerm;
